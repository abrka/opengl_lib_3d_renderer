...
namespace Reflect {
	constexpr entt::hashed_string is_null_func_name{ "is_null" };
	...
	constexpr entt::hashed_string remove_component_func_name{ "remove_component" };
	...
	template<typename T>
	void remove_component_from_entity(entt::registry* reg, entt::entity entity) {
		assert(reg);
		reg->remove<T>(entity);
	}
	template<typename T>
	...
	void register_component(const char* name) {
		...
		entt::meta_factory<T>()
			.type(name)
			.template func<&is_null<T>>(is_null_func_name)
			...
			.template func<&remove_component_from_entity<T>>(remove_component_func_name)
