...
namespace Engine {
	...
	{
		auto entt_view_mesh_infos = entt_registry.view<Engine::MeshInfoComponent>(entt::exclude<Engine::MeshComponent>);
		for (auto [entity, mesh_info] : entt_view_mesh_infos.each()) {
			...
			entt_registry.emplace<Engine::MeshComponent>(entity, Engine::MeshComponent{ result });
		}
	}
}
